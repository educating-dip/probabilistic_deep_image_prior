...
def generate_dist_mtx_torch(side, norm_p=2):
    ...
    dist_mtx = ((coords_exp1 - coords_exp0).abs() ** norm_p).sum(axis=-1) ** (1/norm_p)
    return dist_mtx
      
def RadialBasisFuncCov_torch(side, marg_var, log_lengthscale, device):
    # lengthscale = -1 / log_ar_p
    eps = 1e-5
    dist_mtx = generate_dist_mtx_torch(side, norm_p=1).to(device=device)
...
def linear_posterior_matheron(cov_obs_mat_chol, cov_image_mat, ray_trafo, ray_trafo_adj, ray_trafo_comp, noise_obs_std, mc_samples):
    ...
    f_post_samples = (f_samples - cov_image_mat @ (ray_trafo_adj @ y_post_samples)) # df x Nsamples
    y_non_obs_samples = ray_trafo_comp @ f_post_samples # dy_complement x Nsamples
    return y_non_obs_samples, f_samples, f_post_samples
...
def sampled_EIG(y_samples_per_detector_per_angle, noise_obs_std):
    ...
    angle_cov += (noise_obs_std ** 2) * torch.eye(angle_cov.shape[1], device=angle_cov.device)[None, :, :] # (angles, deterctors, deterctors)
    s, EIG = torch.linalg.slogdet(angle_cov)
    assert all(s == 1)
    return EIG

def x_var(y_samples_per_detector_per_angle, f_post_samples, noise_obs_std):
    # y_samples_per_detector_per_angle -> (angles, deterctors, samples)
    # f_post_samples -> (pixels, samples)

    mc_samples = y_samples_per_detector_per_angle.shape[-1]
    n_angles = y_samples_per_detector_per_angle.shape[0]

    f_sample_inner =  f_post_samples.T @ f_post_samples / mc_samples  # (samples, samples)
    f_sample_inner_expand = f_sample_inner.unsqueeze(0).expand(n_angles, -1, -1)

    angle_cov = torch.bmm(y_samples_per_detector_per_angle, y_samples_per_detector_per_angle.transpose(1,2)) / mc_samples # (angles, deterctors, deterctors)
    angle_cov += (noise_obs_std ** 2) * torch.eye(angle_cov.shape[1], device=angle_cov.device)[None, :, :] # (angles, deterctors, deterctors)
    angle_cov_inv = torch.linalg.inv(angle_cov) # (angles, deterctors, deterctors)
    
    sample_reprojection = torch.bmm( torch.bmm(y_samples_per_detector_per_angle.transpose(1,2), angle_cov_inv), y_samples_per_detector_per_angle) # (angles, samples, samples)
    x_var = torch.bmm(sample_reprojection, f_sample_inner_expand).diagonal( dim1=-2, dim2=-1).sum(dim=-1) / mc_samples # (angles)
    return x_var
...
def optimize_MLL(observation, ray_trafo_module, lr=3e-2, n_steps=1000, patience=10, use_gp_model=False, device='cpu', parameters_init=None):
    ...
    log_prior_var = torch.nn.Parameter(-1.96 * torch.ones(1, device = device))
    parameters = [{'params': log_noise_obs_std}, {'params': log_prior_var}]
    if use_gp_model:
        log_lengthscale = torch.nn.Parameter(1.9637212 * torch.ones(1, device = device))
        parameters[1]['lr'] = 0
        parameters += [{'params': log_lengthscale, 'lr': 0}]
...
def bed_optimal_angles_search_linear_model(
    ...
    for i in tqdm(range(num_batches), miniters=num_batches//100, desc='bed_optimal_angles_search'):
        ...
        s_comp_observations, f_samples, f_post_samples = linear_posterior_matheron(
        ...
        if use_EIG:
            ...
            if use_x_var:
                obj = x_var(s_comp_observations_reshaped, f_post_samples, noise_obs_std).cpu().numpy()
            else:
