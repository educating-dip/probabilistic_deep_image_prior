...
def prior_cov_obs_mat_mul(ray_trafos, filtbackproj, bayesianized_model, hooked_model, be_model, be_modules, v, log_noise_model_variance_obs, masked_cov_grad_tuple=None, use_fwAD_for_jvp=True, add_noise_model_variance_obs=True):
    ...
    if masked_cov_grad_tuple is None: 
    ...
    # computing v_params = v_params * delta_sigma_theta/delta_hyperparams
        ...
        masked_cov_grad_gp_priors, masked_cov_grad_normal_priors = masked_cov_grad_tuple
...
# # multiply v with diag Kyy and + sigma^2_y * v
# def prior_diag_cov_obs_mat_mul(ray_trafos, filtbackproj, bayesianized_model, hooked_model, v, log_noise_model_variance_obs, add_noise_model_variance_obs=True):
...
#     # computing v_params = vAJ
#     v = vec_op_jac_mul_batch(ray_trafos, hooked_model, filtbackproj, v, bayesianized_model)

#     # computing v_params = v * Sigma_theta
#     v_params = vec_weight_prior_cov_mul(bayesianized_model, v)
    
#     return (v_params @ v.T).diag() + torch.exp(log_noise_model_variance_obs)

# build Kyy
def get_prior_cov_obs_mat(ray_trafos, filtbackproj, bayesianized_model, hooked_model, be_model, be_modules, log_noise_model_variance_obs, vec_batch_size, masked_cov_grad_tuple=None, use_fwAD_for_jvp=True, add_noise_model_variance_obs=True):
    ...
    for i in range(0, obs_numel, vec_batch_size):
        ...
        rows_batch = prior_cov_obs_mat_mul(ray_trafos, filtbackproj, bayesianized_model, hooked_model, be_model, be_modules, v, log_noise_model_variance_obs, masked_cov_grad_tuple=masked_cov_grad_tuple, use_fwAD_for_jvp=use_fwAD_for_jvp, add_noise_model_variance_obs=add_noise_model_variance_obs)
...
# # build diag(Kyy) TODO: check! 
# def get_diag_prior_cov_obs_mat(ray_trafos, filtbackproj, bayesianized_model, hooked_model, log_noise_model_variance_obs, vec_batch_size, add_noise_model_variance_obs=True):
#     obs_shape = (1, 1,) + ray_trafos['ray_trafo'].range.shape
#     obs_numel = np.prod(obs_shape)
#     rows = []
#     v = torch.empty((vec_batch_size,) + obs_shape, device=filtbackproj.device)
...
#         # set v.view(vec_batch_size, -1) to be a subset of rows of torch.eye(obs_numel); in last batch, it may contain some additional (zero) rows
#         v.view(vec_batch_size, -1)[:, i:i+vec_batch_size].fill_diagonal_(1.)
#         rows_batch = prior_diag_cov_obs_mat_mul(ray_trafos, filtbackproj, bayesianized_model, hooked_model, v, log_noise_model_variance_obs, add_noise_model_variance_obs=add_noise_model_variance_obs)
#         rows_batch = rows_batch.view(vec_batch_size, -1)
#         if i+vec_batch_size > obs_numel:  # last batch
#             rows_batch = rows_batch[:obs_numel%vec_batch_size]
#         rows.append(rows_batch)
