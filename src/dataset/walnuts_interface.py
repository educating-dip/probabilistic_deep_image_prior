...
def sub_sample_proj_cols(
        projs, factor=DEFAULT_PROJ_COL_SUB_SAMPLING, first=-1):

    if first == -1:
        first = get_first_proj_col_for_sub_sampling(factor=factor)

    out = projs[:, :, first::factor]
    return out

def up_sample_proj_cols(
        projs, factor=DEFAULT_PROJ_COL_SUB_SAMPLING, first=-1,
        kind='linear'):
    ...
    x = np.arange(first, PROJS_COLS, factor)
    projs_interp1d = scipy.interpolate.interp1d(
            x, projs, kind=kind, axis=2, bounds_error=False,
            fill_value=(projs[:, :, 0], projs[:, :, -1]), assume_sorted=True)

    out = projs_interp1d(np.arange(PROJS_COLS))
    return out
...
def get_proj_geom(data_path, walnut_id, orbit_id,
                  ...
                  first_proj_col=-1, num_proj_rows=-1, first_proj_row=-1,
    ...
    # shift the detector center such that the columns match the sub-sampling
    # if it is non-centered
...
def get_projection_data(data_path, walnut_id, orbit_id,
    ...
    # sub-sample projection columns
    projs = sub_sample_proj_cols(
            projs, factor=proj_col_sub_sampling, first=first_proj_col)
...
class WalnutRayTrafo:
    def __init__(self, data_path, walnut_id, orbit_id,
                 ...
                 first_proj_col=-1,
                 num_slices=VOL_SZ[0],
                 num_proj_rows=PROJS_ROWS, first_proj_row=0,
                 rotation=None, shift_z=0.,
                 vol_mask_slice=None, proj_mask_select_k_rows=None,
                 proj_col_sub_sampling_via_geom=True,
                 proj_col_up_sampling_via_geom=True,
                 proj_col_up_sampling_kind_if_not_via_geom='linear'):
        ...
        if self.proj_col_sub_sampling == 1:
            assert self.first_proj_col == 0
        ...
        self.proj_col_sub_sampling_via_geom = (
                proj_col_sub_sampling_via_geom or proj_col_sub_sampling == 1)
        self.proj_col_up_sampling_via_geom = (
                proj_col_up_sampling_via_geom or proj_col_sub_sampling == 1)
        self.proj_col_up_sampling_kind_if_not_via_geom = (
                proj_col_up_sampling_kind_if_not_via_geom)
        ...
        proj_geom_full_kwargs = dict(
                num_proj_rows=PROJS_ROWS,
                first_proj_row=0,
        )

        # geometries with projection column sub-sampling
        self.proj_geom, self.vecs = get_proj_geom(
                **common_proj_geom_kwargs,
                **proj_geom_kwargs,
        ...
        self.proj_geom_full = get_proj_geom(
                **common_proj_geom_kwargs,
                **proj_geom_full_kwargs,
                proj_col_sub_sampling=self.proj_col_sub_sampling,
                first_proj_col=self.first_proj_col)

        # geometries with all projection columns (no sub-sampling)
        if self.proj_col_sub_sampling != 1:
            self.proj_geom_all_cols, self.vecs_all_cols = get_proj_geom(
                    **common_proj_geom_kwargs,
                    **proj_geom_kwargs,
                    proj_col_sub_sampling=1,
                    return_vecs=True)
            self.proj_geom_all_cols_full = get_proj_geom(
                    **common_proj_geom_kwargs,
                    **proj_geom_full_kwargs,
                    proj_col_sub_sampling=1)
        else:
            self.proj_geom_all_cols = None
            self.vecs_all_cols = None
            self.proj_geom_all_cols_full = None
    ...
    def fp3d(self, vol_x, full_input=False):
        ...
        if self.proj_col_sub_sampling_via_geom:
            proj_shape = self.proj_shape
            proj_geom = self.proj_geom
        else:
            proj_shape = self.proj_shape[:2] + (PROJS_COLS,)
            proj_geom = self.proj_geom_all_cols
        ...
        if not self.proj_col_sub_sampling_via_geom:
            projs = sub_sample_proj_cols(
                    projs, factor=self.proj_col_sub_sampling,
                    first=self.first_proj_col)

        return projs

    def bp3d(self, projs, full_input=False):

        if self.proj_col_up_sampling_via_geom:
            proj_geom = (self.proj_geom_full if full_input else
                         self.proj_geom)
        else:
            proj_geom = (self.proj_geom_all_cols_full if full_input else
                         self.proj_geom_all_cols)
            projs = up_sample_proj_cols(
                    projs, factor=self.proj_col_sub_sampling,
                    first=self.first_proj_col,
                    kind=self.proj_col_up_sampling_kind_if_not_via_geom)
