...
def load_or_convert_logs(path, require_num_steps=None):
    ...
    for d in sorted(os.listdir(path)):
        ...
        if d.endswith('DIP+TV') and os.path.isdir(path_d):
...
if __name__ == '__main__':
        ...
        for line in f:
            ...
            if cfg.name == name and ceil(cfg.beam_num_angle / cfg.angular_sub_sampling) == angles and cfg.noise_specs.stddev == noise:
                ...
                if len(psnrs) < cfg.num_images:
                    print('using only {} samples for gamma={}'.format(len(psnrs), cfg.net.optim.gamma))
                print(cfg.net.optim.gamma)
                cfg_list.append(cfg)
                psnrs_list.append(psnrs)
                best_psnrs_list.append(best_psnrs)

    cfg_list, psnrs_list, best_psnrs_list = map(list, zip(*sorted(zip(cfg_list, psnrs_list, best_psnrs_list), key=lambda x: x[0].net.optim.gamma)))
    gammas = [cfg.net.optim.gamma for cfg in cfg_list]
    ...
    for i, (cfg, max_mean_psnr) in enumerate(zip(cfg_list, max_mean_psnr_list)):
        print('gamma {}: max mean psnr {:.2f}'.format(cfg.net.optim.gamma, max_mean_psnr) + (' [max at {} iterations]'.format(np.argmax(np.mean(psnrs_list[i], axis=0))) if i == max_idx else '') + (' [incomplete {}/{}]'.format(len(psnrs_list[i]), cfg.num_images) if len(psnrs_list[i]) < cfg.num_images else ''))

    for cfg, psnrs in zip(cfg_list, psnrs_list):
        print('maximum for gamma {} at {} iterations'.format(cfg.net.optim.gamma, np.argmax(np.mean(psnrs, axis=0))))
