...
from linearized_laplace import compute_jacobian_single_batch, low_rank_GP_lin_pred_cov, sigmoid_gaussian_linearised_log_prob, gaussian_log_prob
...
def coordinator(cfg : DictConfig) -> None:
    ...
    for i, (example_image, _) in enumerate(loader):
        ...
        torch.save(reconstructor.model.state_dict(), './reconstructor_model_{}.pt'.format(i))
        ...
        Jac_x = compute_jacobian_single_batch(filtbackproj, reconstructor.model, example_image.view(-1).shape[0])
        ...
        torch.save(block_priors.state_dict(), './block_priors_no_PredCP_{}.pt'.format(i))

        (_, model_post_cov_no_predCP) = low_rank_GP_lin_pred_cov(block_priors, Jac_x, Jac_y, noise_model_variance_y_no_PredCP)

        trafo_T_trafo = trafo.cuda().T @ trafo.cuda()

        U, S, Vh = tl.truncated_svd(trafo_T_trafo, n_eigenvecs=100)

        lik_hess_inv_no_PredCP = Vh.T @ torch.diag(1/S) @ U.T * noise_model_variance_y_no_PredCP + 5e-4 * torch.eye(U.shape[0], device=block_priors.store_device)

        lik_hess_inv_unit_var = Vh.T @ torch.diag(1/S) @ U.T  + 5e-4 * torch.eye(U.shape[0], device=block_priors.store_device)

        assert lik_hess_inv_no_PredCP.diag().min() > 0

        test_log_lik_no_PredCP = gaussian_log_prob(example_image.flatten().cuda(), torch.from_numpy(recon).flatten().cuda(), model_post_cov_no_predCP, lik_hess_inv_no_PredCP)

        test_log_lik_noise_model_unit_var = gaussian_log_prob(example_image.flatten().cuda(), torch.from_numpy(recon).flatten().cuda(), None, lik_hess_inv_unit_var)

        test_log_lik_noise_model = gaussian_log_prob(example_image.flatten().cuda(), torch.from_numpy(recon).flatten().cuda(), None, lik_hess_inv_no_PredCP)


        print('test_log_lik marginal likelihood optim (no_PredCP): {}'.format(test_log_lik_no_PredCP))
        
        print('test_log_lik likelihood baseline (unit var): {}'.format(test_log_lik_noise_model_unit_var))

        print('test_log_lik likelihood baseline: {}'.format(test_log_lik_noise_model))
        ...
        torch.save(block_priors.state_dict(), './block_priors_w_PredCP_{}.pt'.format(i))

        lik_hess_inv_w_PredCP = Vh.T @ torch.diag(1/S) @ U.T * noise_model_variance_y_w_PredCP + 5e-4 * torch.eye(U.shape[0], device=block_priors.store_device)

        (_, model_post_cov_w_PredCP) = \
            low_rank_GP_lin_pred_cov(block_priors, Jac_x, Jac_y, noise_model_variance_y_w_PredCP)

        test_log_lik_w_PredCP = \
            gaussian_log_prob(example_image.flatten().cuda(), torch.from_numpy(recon).flatten().cuda(),
                                      model_post_cov_w_PredCP, lik_hess_inv_w_PredCP)

        print('log_lik post marginal likelihood optim: {}'.format(test_log_lik_w_PredCP))

        dict = {'cfgs': cfg,
                    'test_log_lik':{
                        'MAP_MLL': test_log_lik_w_PredCP.item(),
                        'MLL_no_PredCP': test_log_lik_no_PredCP.item(),
                        'noise_model': test_log_lik_noise_model_unit_var.item(),
        ...
        np.savez('log_lik_info_{}'.format(i), **dict)
