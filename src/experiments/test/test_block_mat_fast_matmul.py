...
N = 4 # dim_output
P = 6 # dim_params 
A = torch.zeros((N,P)).normal_(0, 1)

# B is a 6*6 block-diagonal matrix with blocks of 2*2 (a.k.a. P^K, P^K where P^K is dim of params in conv kernel)
B = torch.zeros((P,P))
...
C_reshaped = C_reshaped.permute(0, 2, 1).reshape([P, N]).t()
