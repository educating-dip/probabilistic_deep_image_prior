...
def compute_lin_pred_cov(block_priors, Jac, noise_mat_inv):
    ...
    proj_marginal_prior_term =  ( block_priors.matrix_prior_cov_mul(Jac).detach() @ Jac.transpose(1, 0) )
...
def low_rank_GP_lin_pred_cov(block_priors, Jac_x, Jac_y, noise_model_variance_y):
    ...
    Kxx = block_priors.matrix_prior_cov_mul(Jac_x) @ Jac_x.transpose(1, 0)
    Kxy = block_priors.matrix_prior_cov_mul(Jac_x) @ Jac_y.transpose(1, 0)
    Kyx = Kxy.T
