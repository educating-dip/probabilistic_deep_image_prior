...
view_proj_cols_values = [
        slice(None, 40),
        slice(num_proj_cols//2-20, num_proj_cols//2+20),
        slice(-40, None)]
...
for i, angle_index in enumerate(angle_indices):
    ...
    for view_proj_cols in view_proj_cols_values:
        ...
        ax[i, col].set_title('FP of vol mask\ncolumn\n{}'.format(
                range(num_proj_cols)[view_proj_cols]))
        col += 1
    for view_proj_cols in view_proj_cols_values:
        ax[i, col].imshow(
...
for i, (reco, title) in enumerate(zip(recos, titles)):
    ...
    psnr = peak_signal_noise_ratio(gt, reco, data_range=np.max(gt) - np.min(gt))
    ax[0, i].set_xlabel('PSNR: {:f} dB'.format(psnr))
